    ...
    private func setupAudioSession(for category: AVAudioSession.Category = .playAndRecord) {
        ...
        do {
            ...
            if currentAudioSession != category {
                // Configure options based on watchOS version and category
                var options: AVAudioSession.CategoryOptions = []
                
                // allowBluetooth is only available in watchOS 11.0+
                if #available(watchOS 11.0, *) {
                    options.insert(.allowBluetooth)
                }
                
                // defaultToSpeaker is not available on watchOS, use allowBluetoothA2DP instead for playback
